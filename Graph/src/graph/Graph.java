...
public class Graph<T> {
	...
	public Vertex<T> findVertex(T elm) {
		...
		for (Vertex<T> v: set) {
			Vertex<T> foundVertex = DFSearch(v, elm);
...
*/	
	public Vertex<T> DFSearch (Vertex<T> start,T elm){
		...
		setAllVisitedFalse();	
		ArrayDeque<Vertex<T>> stack = new ArrayDeque<>();																	//initalize Q
		stack.push(start);
		start.setVisited(false);											//s.visited = true

		//System.out.print("Search: ");
		while (!stack.isEmpty()) {										//while Q!
			Vertex<T> u = stack.pop();								//remove n from the head of Q
			System.out.print(u.getData() + " ");
				 
				if (u.getData().equals(elm)){								//U CONTAINS T
				System.out.println();
				return u;
				}//RETURN N
				...
				for (Edge<T> e: list) {										//FOR EACH ADJ
					Vertex<T> v = e.getAdjacentVertex(u);
					if (v.getVisited() == false) {							//IF VISITED
						v.setVisited(true);									//SET VTIED = TRUE
						stack.push(v);										// ADD V TO THE TAIL OF Q
					}
				}
